document.addEventListener("DOMContentLoaded", () => {
    ...
    closeLogin.onclick = () => { loginModal.style.display="none"; loggedOut=true; }
    ...
    if(SpeechRecognition){
        ...
        recognition.onerror = e => { micBtn.innerHTML = "🎤"; alert("Microphone error: "+e.error); };

    } else { micBtn.disabled = true; micBtn.title = "Your browser does not support microphone input."; }
    ...
    function botReply(userText){
        ...
        if(text.includes("html")) return "HTML structures the content of web pages.";
        if(text.includes("css")) return "CSS styles and designs websites.";
        if(text.includes("javascript")) return "JavaScript adds logic and interaction to websites.";
        if(text.includes("frontend")) return "Frontend is the part of a website users can see and interact with.";
        if(text.includes("backend")) return "Backend handles server, database, and logic behind the scenes.";
